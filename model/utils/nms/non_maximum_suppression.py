...
def non_maximum_suppression(bbox, thresh, score=None, limit=None):
    """
    通过IOU进行非极大值抑制
    :param bbox: (R,4) y_min, x_min, y_max, x_max
    :param thresh: IOU 阈值
    :param score: confidence的分数
    :param limit: 输出结果的上限
    :return:(K,)
    """
    return _non_maximum_suppression(bbox, thresh, score=None, limit=None)


def _non_maximum_suppression(bbox, thresh, score=None, limit=None):
    if len(bbox) == 0:
...
def _call_nms_kernel(bbox, thresh):
    n_bbox = bbox.shape[0]
    thresh_per_block = 64
    col_blocks = np.ceil(n_bbox / thresh_per_block).astype(np.int32)
    blocks = (col_blocks, col_blocks, 1)
    threads = (thresh_per_block, 1, 1)
    ...
    selection, n_selec = _nms_gpu_post(
        mask_host, n_bbox, thresh_per_block, col_blocks
    )
